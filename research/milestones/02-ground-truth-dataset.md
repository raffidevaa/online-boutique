# Phase 02 — Ground truth dataset collection
...
- Phase 01 done — scenario runner + ground truth logger validated on at least one service.

## Steps

1. **Define the experiment matrix.** Cross the fault taxonomy categories (resource, network,
   container, dependency) against the target service list in `FAULT_TAXONOMY.md`. Exclude
...
3. **Log every experiment** using `research/docs/EXPERIMENT_LOG.md` as the template, saved to
...
6. **Check coverage.** Make sure every target service has at least one logged run for each
   fault category that applies to it (dependency-level faults naturally concentrate on
   `checkoutservice` and its callees, per `FAULT_TAXONOMY.md`).

## Deliverables

- `research/experiments/<experiment_id>.md` — one file per run
