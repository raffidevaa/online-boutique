...
- Phase 02 done — full ground truth dataset collected.
- Phase 04 done — agent produces RCA + remediation pairs.

## Steps

1. **Define metrics:**
   - RCA accuracy — match rate of agent's root cause vs. `expected_root_cause`.
...
2. **Build an evaluation script** that reads the phase-02 dataset index +
   `research/experiments/*.md`, runs the agent against each recorded incident window, and
...
4. **Separate confounder-affected runs.** Report `adservice`/`shoppingassistantservice`
   -flagged runs separately from the clean set, per the flags set in phase 02 — don't let
...
- Metrics computed across the entire phase-02 dataset.
